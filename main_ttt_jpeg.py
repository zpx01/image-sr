...
import time
import random
from data_loader import DataLoaderJPEG
import cv2
from torch.optim.lr_scheduler import ReduceLROnPlateau
import math
import os
from ttt_dataloader import *
...
def get_args_parser():
    parser = argparse.ArgumentParser('Test Time Training - JPEG Compression', add_help=False)
...
def init_test_time(args, model_path, optimizer_path, device):
        # instantiate model
        model = net(upscale=1, in_chans=1, out_chans=1, img_size=126, window_size=7, img_range=255.0, 
...
def main(args):
    ...
    if args.no_opt:
        ...
        optimizer = torch.load(args.opt_path)
    ...
    for idx, path in enumerate(sorted(files)):
        ...
        print(f'SAVE_DIR: {save_dir}/{img_name}.pth')
        if os.path.exists(f'{save_dir}/{img_name}.pth'):
            continue
        data_loader = DataLoaderJPEG(path, jpeg_quality=args.quality, num_imgs=10)
        data_loader = torch.utils.data.DataLoader(data_loader, batch_size=args.batch_size, shuffle=True)
        if not args.zero_loss:
            for epoch in range(args.epochs):
                ...
                if (epoch) % args.save_freq == 0:
                    torch.save(state_dict['state_dict'], f'{save_dir}/{img_name}.pth')
        else:
            epoch = 0
            loss = float('inf')
            adjust_learning_rate(optimizer, epoch)
            ...
            # while not has_loss_plateaued(loss_values, threshold=0.0001, lookback=12) and epoch <= 200:
